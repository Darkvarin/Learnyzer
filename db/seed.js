...
async function seed() {
    try {
        ...
        const existingTutor = await index_1.db.query.aiTutors.findFirst();
        if (!existingTutor) {
            for (const tutor of aiTutors) {
                await index_1.db.insert(schema.aiTutors).values(tutor);
            }
        ...
        const existingTool = await index_1.db.query.aiTools.findFirst();
        if (!existingTool) {
            for (const tool of aiTools) {
                await index_1.db.insert(schema.aiTools).values(tool);
            }
        ...
        const existingCourseCategory = await index_1.db.query.courseCategories.findFirst();
        if (!existingCourseCategory) {
            for (const category of courseCategories) {
                await index_1.db.insert(schema.courseCategories).values(category);
            }
        ...
        const allCourses = await index_1.db.query.courses.findMany();
        // Delete school-type courses and track other courses to avoid duplicates
        const existingTitles = new Set();
        let deletedCount = 0;
        for (const course of allCourses) {
            if (course.examType === "School") {
                await index_1.db.delete(schema.courses).where((0, drizzle_orm_1.eq)(schema.courses.id, course.id));
                deletedCount++;
            }
        ...
        for (const course of entranceExamCourses) {
            if (!existingTitles.has(course.title)) {
                await index_1.db.insert(schema.courses).values(course);
                addedCount++;
                existingTitles.add(course.title);
            }
        }
        ...
        const existingStreakGoal = await index_1.db.query.streakGoals.findFirst();
        if (!existingStreakGoal) {
            for (const goal of streakGoals) {
                await index_1.db.insert(schema.streakGoals).values(goal);
            }
        ...
        const existingAchievement = await index_1.db.query.achievements.findFirst();
        if (!existingAchievement) {
            for (const achievement of achievements) {
                await index_1.db.insert(schema.achievements).values(achievement);
            }
        ...
        const existingReward = await index_1.db.query.rewards.findFirst();
        if (!existingReward) {
            for (const reward of rewards) {
                await index_1.db.insert(schema.rewards).values(reward);
            }
        ...
        const existingFeedbackCategory = await index_1.db.query.feedbackCategories.findFirst();
        if (!existingFeedbackCategory) {
            for (const category of feedbackCategories) {
                await index_1.db.insert(schema.feedbackCategories).values(category);
            }
        ...
        const existingPowerUp = await index_1.db.query.powerUps.findFirst();
        if (!existingPowerUp) {
            for (const powerUp of powerUps) {
                await index_1.db.insert(schema.powerUps).values(powerUp);
            }
